...
!  integer ivb,ivs,ivf
  ...
  do ivb=1,3 ! third order Runge-Kutta
  ...
  CALL findzall ! because of moving free surface, need to recalculate z positions
  CALL sigma 
  CALL advection_and_mixing(ivs,ivf,dtim,step) 
  CALL intpol 
 if(.NOT.(use_Shchepetkin)) then 
    CALL rpevalgrad_Song(ivf) ! baroclinic pressure gradient
   else
  ...
  CALL hsolve(h,oldh,hdt,dtim) ! free surface position
  CALL calcskfc                 
  CALL vhydro(dtim)             ! hydrostatic velocity
  CALL cfdiv(cfcdiv)   
  CALL newcor(dtim,ivs)           
  CALL newsrc                   
  edt= EPS/dtim 
  CALL mgrid(pcorr,dtim,edt,cfcdiv) ! pressure in 3D
  CALL vface(pcorr,dtim)          ! after non-hydrostatic pressure added
  CALL vcenter(pcorr,dtim,ivf)       
  IF (fnhhy.NE.0) CALL pcorrect(pcorr)
  CALL facediv(dtim,fdiv)             
  CALL cdiv(dtim,ctrdiv,ivf)
  CALL tracersource_nut(ivs,ivf,dtim)
