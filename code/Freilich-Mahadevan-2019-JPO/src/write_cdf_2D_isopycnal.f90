subroutine write_cdf_2D_isopycnal(sigma,counter_2d,n)
  !     ------------------------------------------------------------------
USE header,ONLY : NI,NJ,NK,ntr,DL,LEN,stress_top_x,T,Tr,rho,xc,yc,zc,zf,vor,strain,shear,freqN2,nconsume,time_seconds,dirout,rc_kind,u,v,w,consump
...
#include "netcdf.inc"                                                                        
     ...
     &     usurf(NI,NJ,ntr), vsurf(NI,NJ,ntr), wsurf(NI,NJ,ntr),consumpsurf(NI,NJ,ntr), &
     &     ugsurf(NI,NJ,ntr), vgsurf(NI,NJ,ntr)
 ...
 integer :: iddatfile, idigit, idudx, idudy, idudz, idvbysection, idvby, idvbz, idvcon, idvcy, idvbx
  ...
  integer :: idvstrain,idvstress,idvstr,idvs,idvtbar,idvtemp,idvtim,idvtr,idvt,idvu,idvvb,idvvc,idvvor,idvv,idvug,idvvg
...
!     end do 
     ...
     &                 sigdn*Tr(1,i,j,k-1,n)
                  consumpsurf(i,j,it)=sigup*consump(i,j,k,1) +      &
     &                 sigdn*consump(i,j,k,1)
                  usurf(i,j,it)= sigup*u(i,j,k,n) +                  &
     &                 sigdn*u(i,j,k-1,n)
                  ugsurf(i,j,it)= sigup*ugeo(i,j,k) +              &
     &                 sigdn*ugeo(i,j,k-1)
                  vsurf(i,j,it)= sigup*v(i,j,k,n) +                  &
     &                 sigdn*v(i,j,k-1,n)
                  vgsurf(i,j,it)= sigup*vgeo(i,j,k) +              &
     &                 sigdn*vgeo(i,j,k-1)
                  wsurf(i,j,it)= sigup*w(i,j,k,n) +                  &
     &                 sigdn*w(i,j,k-1,n)
                   goto 101
               end if
            end do
!     If NK reached and rho above not less than sig, then sig is at surf
            ...
            consumpsurf(i,j,it)= consump(i,j,NK,1)
            usurf(i,j,it)= u(i,j,NK,n)
            ugsurf(i,j,it)= ugeo(i,j,NK)
            vsurf(i,j,it)= v(i,j,NK,n)
            vgsurf(i,j,it)= vgeo(i,j,NK)
            wsurf(i,j,it)= w(i,j,NK,n)
  101    end do
      end do
      end do
     ...
     WRITE(outname,'("isopycnal_",I9.9,".cdf")') INT(sigma*1000.)
      ...
      if (counter_2d.eq.1) then 
         ...
         idvcon = ncvdef(idDatFile,'con',NCFLOAT,4,dimstr,rcode)
     ...
     &        rcode)
         idvu = ncvdef(idDatFile,'u',NCFLOAT,4,dimstr,rcode)
         idvug = ncvdef(idDatFile,'ugeo',NCFLOAT,4,dimstr,rcode)
         idvv = ncvdef(idDatFile,'v',NCFLOAT,4,dimstr,rcode)
         idvvg = ncvdef(idDatFile,'vgeo',NCFLOAT,4,dimstr,rcode)
         idvw = ncvdef(idDatFile,'w',NCFLOAT,4,dimstr,rcode)
         ...
         idvcon = NCVID(idDatFile, 'con', RCODE)
     ...
     &     stresssurf,rcode)
      CALL ncvpt(idDatFile,idvu, starttr, counttr, usurf, rcode)
      CALL ncvpt(idDatFile,idvug, starttr, counttr, ugsurf, rcode)
      CALL ncvpt(idDatFile,idvv, starttr, counttr, vsurf, rcode)
      CALL ncvpt(idDatFile,idvvg, starttr, counttr, vgsurf, rcode)
      CALL ncvpt(idDatFile,idvw, starttr, counttr, wsurf, rcode)
