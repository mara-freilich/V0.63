...
!------------------------------------------------------------------------!
  ...
  REAL(kind=rc_kind), dimension(    0:NI+1,0:NJ+1, 0:NK+1)      :: zc,wx,wy,wz,p,strain,shear,Jac,Jacinv,rho,rho_bar,rho_pri,vor,pv,divQ
...
REAL(kind=rc_kind) :: sasouth(NK),Tsouth(NK),snorth(NK),Tnorth(NK), smid(NK), Tmid(NK)
