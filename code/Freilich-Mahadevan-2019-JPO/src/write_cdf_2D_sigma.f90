...
!                                          
 ...
 integer :: iddatfile, idigit, idudx, idudy, idudz, idvbysection, idvby, idvbz, idvcon, idvcy, idvbx
  ...
  integer :: idvstrain,idvstress,idvstr,idvs,idvtbar,idvtemp,idvtim,idvtr,idvt,idvu,idvvb,idvvc,idvvor,idvv,idvug,idvvg,idvdivQ
...
!            ysurf(i,j)= yc(j)                                          
            ...
            ugsurf(i,j)= ugeo(i,j,ksurf)
            vsurf(i,j)= v(i,j,ksurf,n)
            vgsurf(i,j)= vgeo(i,j,ksurf)
...
!         dimstr(4) = ncddef(idDatFile,'time',NCUNLIM,rcode)            
         ...
         idvug = ncvdef(idDatFile,'ugeo',NCFLOAT,3,dims,rcode)
         idvv = ncvdef(idDatFile,'v',NCFLOAT,3,dims,rcode)
         idvvg = ncvdef(idDatFile,'vgeo',NCFLOAT,3,dims,rcode)
...
!         idvb = NCVID(idDatFile, 'integconsBBL', RCODE)  
      ...
      CALL ncvpt(idDatFile,idvug, start, count, ugsurf, rcode)
      CALL ncvpt(idDatFile,idvv, start, count, vsurf, rcode)
      CALL ncvpt(idDatFile,idvvg, start, count, vgsurf, rcode)
