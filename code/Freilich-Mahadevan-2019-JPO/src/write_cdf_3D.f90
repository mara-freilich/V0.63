...
!----------------------------------------------------------             
  ...
  USE header, only : NI,NJ,NK,xc,yc,zc,p,h,consump,T,s,rho,Tr,u,v,w,vor,pv,uf,vf,wf,Kz,conv,con100,nconsume,dirout,rc_kind,ntr,freqN2
#include "netcdf.inc"                                                                        
  ...
  do j=0,NJ+1 
    do i=0,NI+1 
	  ...
	  freqN2(i,j,NK+1) = freqN2(i,j,NK) ; freqN2(i,j,0) = freqN2(i,j,1) ; 
      do it=1,ntr 
  ...
  idvn2 = ncvdef(idDatFile,'n2',NCshort,3,dims,rcode)
  ...
  CALL ncvpt(idDatFile,idvn2, start, count, freqN2, rcode)
