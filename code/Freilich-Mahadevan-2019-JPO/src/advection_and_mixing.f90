...
do selvar=1,5+ntr
  ...
  if(av_comp(selvar)==1) then
     is_sink = 0     ! Flag that indicates when to add a sinking velocity
    ...
    do it=1,ntr
      ...
      if(selvar==7) then; is_sink=1; elseif(selvar==9) then; is_sink = 1; else; is_sink=0; endif
    enddo

!PRINT *, "var", selvar, "flag", is_sink
    ...
    do it=1,ntr
      ...
      if(selvar==7) Tr(2,1:NI,1:NJ,NK,n) = 1.d-3 !!!!!***** add to top b/c of sinking
    enddo
