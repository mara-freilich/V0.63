...
REAL(kind=rc_kind), PARAMETER :: courinv=2.d0, wi_sink=10.0/86400. , D_sink= 1000.     !wi_sink is in m/s
...
REAL(kind=rc_kind) :: wf_sink(0:NK),wfface                           ! wf_sink is the face sinking velocity of particles
    ...
    do i=1,NI 
      ...
      if (is_sink.eq.1) then
         ...
         do k=1,NK-1
            ...
            zdep= dmax1(1., -zf(i,j,k)*DL /D_sink )   ! zdep is positive, and it's max value should be 1
