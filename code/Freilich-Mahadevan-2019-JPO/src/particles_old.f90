...
MODULE particles
  USE header, ONLY : NI,NJ,NK,uf,vf,wf,Jifc,Jjfc,J2d,ux,vy,NPR,wz,PI,dtf,vor,shear,rho,strain,zc,zf,&
                    &s,parti_file_num,DL,rc_kind, pcx, pcy, pcz, pcr, dirout, divQ, Tr,WL

  ! define the class for particles
  TYPE particle
     REAL(kind=rc_kind) ::  i,j,k,x,y,z,u,v,w,s,t,u0,v0,w0,id,vor,strain,shear,rho,time,divQ,TrN, TrP
  END TYPE particle

  TYPE (particle), DIMENSION(:), ALLOCATABLE :: parti
  REAL(kind=rc_kind) ::  dz,swap1,swap2,swap3,wtotal,wsink
...
CONTAINS
  ...
  SUBROUTINE ini_particles(time)
    ...
    REAL(kind=rc_kind) ::  rand,r,theta, x1, y1, ak, aj
  ...
  SUBROUTINE get_parti_vel(time)
    ...
    DO ip = 1, NPR
       ...
       IF (parti(ip)%j < NJ .AND. parti(ip)%j > 0 .AND. &
          ...
          !CALL interp_trilinear(dic,djc,dz,ugeo(ic:ic+1,jc:jc+1,kc:kc+1),parti(ip)%ugeo)
          !CALL interp_trilinear(dic,djc,dz,vgeo(ic:ic+1,jc:jc+1,kc:kc+1),parti(ip)%vgeo)
          CALL interp_trilinear(dic,djc,dz,divQ(ic:ic+1,jc:jc+1,kc:kc+1),parti(ip)%divQ)
          CALL interp_trilinear(dic,djc,dz,Tr(1,ic:ic+1,jc:jc+1,kc:kc+1,0),parti(ip)%TrN)
          CALL interp_trilinear(dic,djc,dz,Tr(2,ic:ic+1,jc:jc+1,kc:kc+1,0),parti(ip)%TrP)
!!$          if (ip == 8001) then
!!$          print*, 'dkc,dz=',dkc,dz
...
!!$          endif
  ...
  SUBROUTINE parti_forward()
    IMPLICIT NONE
    INTEGER :: i
    REAL(kind=rc_kind), dimension(NPR) :: wsink
        wsink(1:NPR/2) = 0.
        wsink(NPR/2+1:NPR) = 1./86400/WL
        !wsink(2*NPR/3+1:NPR) = 0.
    DO i = 1, NPR
       ...
       wtotal = parti(i)%w-wsink(i)
       IF (parti(i)%k>NK-1 .AND. wtotal>0) THEN
          parti(i)%k = parti(i)%k + wtotal * dtf / (1d0 + (wtotal * dtf)/(DBLE(NK) - parti(i)%k) )
       ELSE IF (parti(i)%k<1 .AND. wtotal<0) THEN
          parti(i)%k = parti(i)%k + wtotal * dtf / ( 1d0 - dtf/parti(i)%k )
       ELSE
          parti(i)%k = parti(i)%k + 0.5d0 * dtf * (3d0 * wtotal - parti(i)%w0)
       ENDIF
