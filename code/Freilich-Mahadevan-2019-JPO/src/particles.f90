...
MODULE particles
  USE header, ONLY : NI,NJ,NK,uf,vf,wf,Jifc,Jjfc,J2d,ux,vy,NPR,wz,PI,dtf,vor,shear,rho,strain,zf,&
                    &s,T,parti_file_num,DL,rc_kind,pcx,pcy,pcz,pcr,dirout,divQ,dx,dy,WL,EPS,pfac,&
                    &dztop,D,UL,freqN2,Tr

  ! define the class for particles
  TYPE particle
     REAL(kind=rc_kind) ::  i,j,k,x,y,z,u,v,w,s,t,u0,v0,w0,id,vor,strain,shear,rho,time,divQ,wtotal,wsink,wzf,N2,N,P
  END TYPE particle
...
! This subroutine writes the listed variables for each of the particle.
  SUBROUTINE save_parti()
    IMPLICIT NONE
    INTEGER :: i_file,ip
    INTEGER :: i,j,k,ic,jc,kc,ifc,jfc,kfc,time
    ...
    DO ip = 1, NPR
         ...
         dkc = parti(ip)%k - kc + 0.5d0
          ...
          ! Diagnose future step properties
      	!CALL interp_trilinear(dic,djc,dkc,rho(ic:ic+1,jc:jc+1,kc:kc+1),parti(ip)%rho0)
      	!CALL interp_trilinear(dic,djc,dkf,freqN2(ic:ic+1,jc:jc+1,kfc:kfc+1),parti(ip)%N20)
    ENDDO
       ...
       DO i_file = 1, parti_file_num
          DO ip = (i_file - 1) * NPR_eachfile + 1, i_file * NPR_eachfile
             WRITE(file_id(i_file)) parti(ip)%id, &    ! Particle number
                  ...
                  parti(ip)%divQ, &          ! Frontogenetic function
 ...
 !     DO i_file = 1, parti_file_num
 !       DO ip = (i_file - 1) * NPR_eachfile + 1, i_file * NPR_eachfile
 !         WRITE(file_id(i_file)) parti(ip)%rho0, &          ! past density
 !           parti(ip)%N20              ! past N^2
 !         ENDDO
 !       ENDDO

 !   END SELECT

 ! END SUBROUTINE save_parti_half
...
! to all variables, then assigns the user-defined distribution to the particles.
  SUBROUTINE ini_particles(time)
    IMPLICIT NONE
    INTEGER :: time,ip,i,j,k
    REAL(kind=rc_kind) ::  range,ai,aj,sig
    logical :: exist
    ...
    DO ip=1, NPR
	   ...
	   parti(ip)%P = 0d0
    ENDDO
    PRINT*, "# finish initial particles' velocities"
        ...
        do i=1,NI
            do k=2,NK
                if (rho(i,j,k).le.sig) then
                    ...
                    zs(i,j)= sigup*k +sigdn*(k-1)
                    goto 101
                end if
            end do
!     If NK reached and rho above not less than sig, then sig is at surface
            zs(i,j)= NK
        101 end do
    end do

    DO i=1,65000
        parti(i)%i = REAL(MOD(i,NI))
        IF (MOD(i,NI).eq.0) THEN
            parti(i)%i=REAL(NI)
            aj = aj+1
        ENDIF
        parti(i)%j=REAL(MOD(INT(aj),INT(NJ)))
        IF (MOD(INT(aj),INT(NJ)).eq.0) THEN
            parti(i)%j=NJ
        ENDIF
        parti(i)%k=zs(INT(parti(i)%i),INT(parti(i)%j));
        ...
        ! Save the seeding position
        inquire(file=TRIM(dirout)//'seeding.out', exist=exist)
        if (exist) then
            open(12, file=TRIM(dirout)//'seeding.out', status="old", position="append", action="write")
        else
            open(12, file=TRIM(dirout)//'seeding.out', status="new", action="write")
            WRITE(12,*) "ip, ","parti(ip)%i, ", "parti(ip)%j, ", "parti(ip)%k"
        end if
        WRITE(12,*) ip, ",", parti(ip)%i, ",", parti(ip)%j
        close(12)
    ...
    DO k = 0, NK-2
        DO i = 0, NI+1
            DO j = 0, NJ+1
                ...
                wzf(i,j,k) = 1/mycoef*exp(pfac/(NK-1)*k)
            ENDDO
        ENDDO
    ENDDO
    ...
    DO ip = 1, NPR
       ...
       IF (parti(ip)%j <= NJ .AND. parti(ip)%j >= 0 .AND. &
          ...
          ELSE
              ...
              ! Linear interpolation if above NK-1 k-level
              CALL interp_trilinear(dic,djc,dkf,wzf(ic:ic+1,jc:jc+1,kc:kc+1),parti(ip)%wzf)
...
!          parti(ip)%wsink= -0d0/86400d0/WL*parti(ip)%wzf*EPS  ! 0 m/day
          ...
          !ELSE IF (ip > NPR/3 .AND. ip <= 2*NPR/3) THEN
          !    parti(ip)%wsink= -10d0/86400d0/WL*parti(ip)%wzf*EPS  ! 10m/day
          !ELSE IF (ip > 2*NPR/3 .AND. ip <= 3*NPR/3) THEN
          !    parti(ip)%wsink= -50d0/86400d0/WL*parti(ip)%wzf*EPS  ! 50m/day
          !END IF
		  ...
		  CALL interp_trilinear(dic,djc,dkc,Tr(1,ic:ic+1,jc:jc+1,kc:kc+1,1),parti(ip)%N)
		  CALL interp_trilinear(dic,djc,dkc,Tr(2,ic:ic+1,jc:jc+1,kc:kc+1,1),parti(ip)%P)
          CALL interp_trilinear(dic,djc,dkf,freqN2(ic:ic+1,jc:jc+1,kfc:kfc+1),parti(ip)%N2)
